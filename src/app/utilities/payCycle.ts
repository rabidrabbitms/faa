...
export class DateCycle {
    constructor(init?:Partial<DateCycle>){
        if (init){
            if (init.Type)
            this.Type = init.Type;
            if (init.Frequency)
    ...
    Type:DateCycleType = DateCycleType.Weekly;
    Frequency:number = 1;
    References:DateCycleReference[] = [new DateCycleReference()];
}
...
export class DateCycleReference {    
    constructor(init?:Partial<DateCycleReference>){
        if (init){
            if (init.Type)
                this.Type = init.Type;
            if (init.Date)
...
export class PayCycle extends DateCycle{
    constructor(init:Partial<DateCycle>){
        super(init);
    }
